...
const SearchField = React.forwardRef<HTMLInputElement, SearchFieldProps>(
  (
    { className, "aria-label": ariaLabel = "Поиск по трекам", ...props },
    ref,
  ) => {
    return (
      <div
        className={cn("search-field", "relative flex flex-1 group", className)}
      >
        <span className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center justify-center text-[#4e4e4e] transition-colors group-focus-within:text-white">
          <Search
