...
/* Тот же размер (15px паддинг по вертикали); смещено вниз за счёт нижнего паддинга — текст по центру */
...
/* По макету: .main__filter-list — height calc(5*1.2em+4*28px), gap 28px; +1px чтобы не было скролла */
...
/* По макету: ховер — только цвет #D9B6FF; подсветка — бледно-розовый фон. ! перебивает focus:bg-accent из dropdown-menu */
...
export interface FilterSelectPropsMulti {
  ...
  triggerLabel: string
  /** Горизонтальное расположение пунктов (по умолчанию вертикально) */
  horizontal?: boolean
  /** Показывать чекбоксы (по умолчанию скрыты) */
...
export interface FilterSelectPropsSingle {
  ...
  triggerLabel: string
  /** Горизонтальное расположение пунктов */
  horizontal?: boolean
  /** Показывать радио-индикаторы (по умолчанию скрыты) */
  showControls?: boolean
  "aria-label"?: string
  className?: string
}

export type FilterSelectProps = FilterSelectPropsMulti | FilterSelectPropsSingle
...
export function FilterSelect(props: FilterSelectProps) {
  ...
  /* Контрол впритык слева (left-0), ширина 19px; между контролом и текстом 12px → pl 19+12=31px */
  ...
  /* Точка при выборе — HTML/CSS круг (ItemIndicator в dropdown-menu), не градиент */
  const radioControlClasses = showControls
    ...
    : "[&>span]:hidden pl-0 pr-0"
  /* По макету: вертикальный список gap 28px; год — .main__filter-year gap 24px */
  ...
  const triggerAriaLabel = isMulti
    ? ariaLabel ?? (count ? `${triggerLabel}, ${count} в наборе` : triggerLabel)
    : ariaLabel
    ...
    <div className={cn("relative", className)}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className={filterTriggerClasses} aria-label={triggerAriaLabel}>
            {triggerLabel}
          </Button>
        </DropdownMenuTrigger>
        {showBadge && (
